...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.dh_handler, effect_src_receiver.get_parent().name + " has a BurningHandler but no DHHandler.")
...
func handle_burning(burning_effect: BurningEffect) -> void:
	...
	var burning_weakness: float = effect_src_receiver.affected_entity.sc.get_stat("burning_weakness")
	var burning_resistance: float = effect_src_receiver.affected_entity.sc.get_stat("burning_resistance")
	...
	(effect_src_receiver.get_node("DHHandler") as DHHandler).handle_over_time_dmg(local_dot_resource, burning_effect.get_full_effect_key())
