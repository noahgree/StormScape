...
var source_entity: Entity = null ## The source entity of the most recent effect source. Updated by the esi receiver.
...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.dh_handler, effect_src_receiver.get_parent().name + " has a LifeStealHandler but no DHHandler.")
...
func handle_life_steal(dmg_amount: int, percent_to_steal: float) -> void:
	if source_entity != null:
		...
		var self_health_component: HealthComponent = effect_src_receiver.get_parent().get_node("HealthComponent")
