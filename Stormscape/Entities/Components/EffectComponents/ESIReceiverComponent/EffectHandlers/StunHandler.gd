...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.get_parent() is DynamicEntity, effect_src_receiver.affected_entity.name + " has an esi receiver intended to handle stuns, but the affected entity is not a DynamicEntity.")
...
func handle_stun(stun_effect: StunEffect) -> void:
	var stun_weakness: float = effect_src_receiver.affected_entity.sc.get_stat("stun_weakness")
	var stun_resistance: float = effect_src_receiver.affected_entity.sc.get_stat("stun_resistance")
...
func _send_handled_stun(stun_time: float) -> void:
	if stun_time > 0:
		effect_src_receiver.affected_entity.request_stun(stun_time)
