...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.heal_handler, effect_src_receiver.get_parent().name + " has a RegenHandler but no HealHandler.")
...
func handle_regen(regen_effect: RegenEffect) -> void:
	if regen_effect.hot_resource != null: # Needed for when we nullify on game load
		...
		var regen_boost: float = effect_src_receiver.affected_entity.sc.get_stat("regen_boost")
		var regen_penalty: float = effect_src_receiver.affected_entity.sc.get_stat("regen_penalty")
		...
		effect_src_receiver.heal_handler.handle_over_time_heal(local_hot_resource, regen_effect.get_full_effect_key())
