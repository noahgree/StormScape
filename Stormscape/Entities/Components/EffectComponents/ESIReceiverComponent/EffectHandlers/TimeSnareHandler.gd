...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.get_parent() is DynamicEntity, effect_src_receiver.affected_entity.name + " has an esi receiver intended to handle time snares, but the affected entity is not a DynamicEntity.")
...
func handle_time_snare(time_snare_effect: TimeSnareEffect) -> void:
	if effect_src_receiver.affected_entity.sc.get_stat("time_snare_immunity") > 0:
		effect_src_receiver.condition_manager.request_effect_removal_for_all_sources("time_snare")
		return

	if effect_src_receiver.affected_entity is DynamicEntity:
		effect_src_receiver.affected_entity.request_time_snare(time_snare_effect.time_multiplier, time_snare_effect.snare_time)
