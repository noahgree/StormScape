...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.dh_handler, effect_src_receiver.get_parent().name + " has a FrostbiteHandler but no DHHandler.")
...
func handle_frostbite(frostbite_effect: FrostbiteEffect) -> void:
	...
	var frostbite_weakness: float = effect_src_receiver.affected_entity.sc.get_stat("frostbite_weakness")
	var frostbite_resistance: float = effect_src_receiver.affected_entity.sc.get_stat("frostbite_resistance")
	...
	(effect_src_receiver.get_node("DHHandler") as DHHandler).handle_over_time_dmg(local_dot_resource, frostbite_effect.get_full_effect_key())
