...
func initialize(receiver: ESIReceiverComponent) -> void:
	...
	assert(effect_src_receiver.dh_handler, effect_src_receiver.get_parent().name + " has a PoisonHandler but no DHHandler.")
...
func handle_poison(poison_effect: PoisonEffect) -> void:
	if poison_effect.dot_resource != null: # Needed for when we nullify on game load
		...
		var poison_weakness: float = effect_src_receiver.affected_entity.sc.get_stat("poison_weakness")
		var poison_resistance: float = effect_src_receiver.affected_entity.sc.get_stat("poison_resistance")
		...
		(effect_src_receiver.get_node("DHHandler") as DHHandler).handle_over_time_dmg(local_dot_resource, poison_effect.get_full_effect_key())
