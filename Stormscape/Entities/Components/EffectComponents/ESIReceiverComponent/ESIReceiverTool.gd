@tool
## Helps manage auto assigning export references to nodes added as children of an ESIReceiverComponent.


func update_editor_children_exports(node: ESIReceiverComponent, children: Array[Node]) -> void:
	for child: Node in children:
		if child is DHHandler:
			node.dh_handler = child
		if child is HealHandler:
			node.heal_handler = child

func ensure_effect_handler_resource_unique_to_scene(node: ESIReceiverComponent) -> void:
...
func update_editor_parent_export(node: ESIReceiverComponent, parent: Node) -> void:
	if parent is Entity:
		node.affected_entity = parent
	if parent.has_node("ConditionsComponent"):
		parent.get_node("ConditionsComponent").effect_src_receiver = node

func _init(node: ESIReceiverComponent) -> void:
