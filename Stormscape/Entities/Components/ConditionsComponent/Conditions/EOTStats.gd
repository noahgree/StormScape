...
@export var perpetual: bool = false ## When true, the esi from the first element in the esi array will continue to be applied until the source condition is removed. [color=cyan] If part of a condition added in an AOE, this should probably be set to true![/color]
...
@export var ticks_array: Array[TickES] ## The tick effect sources to apply at each tick. Using only one is recommended, but if more than one are used, the amount used should match the tick count.
