...
static func create_popup(popup_type: POPUP_TYPE, popup_value: int, node: Entity, esi: ESI) -> EffectPopup:
	var popup: EffectPopup = popup_scene.instantiate()
	Globals.world_root.add_child(popup)
	popup.parent_node = node

	popup.set_as_new(popup_type, popup_value, esi)

	return popup

func set_as_new(popup_type: POPUP_TYPE, popup_value: int, esi: ESI) -> void:
	...
	value = 0
	update_popup(popup_type, popup_value)
