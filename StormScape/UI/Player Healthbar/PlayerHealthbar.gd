...
var previous_shield: int
var previous_hunger: int
var previous_stamina: float
var stamina_recharge_tween: Tween


func _ready() -> void:
	SignalBus.PlayerHealthChanged.connect(on_health_changed)
	SignalBus.PlayerShieldChanged.connect(on_shield_changed)
	SignalBus.PlayerHungerChanged.connect(on_hunger_changed)
	SignalBus.PlayerStaminaChanged.connect(on_stamina_changed)
	SignalBus.PlayerHungerTicksChanged.connect(on_hunger_ticks_changed)
	SignalBus.PlayerStaminaWaitTimerStateChange.connect(on_player_stamina_wait_timer_state_change)
...
func on_stamina_changed(new_stamina: float) -> void:
	...
	stamina_bar.material.set_shader_parameter("progress", 1.0 - new_stamina / 100.0)

func on_hunger_ticks_changed(new_hunger_ticks: float) -> void:
	hunger_progress_bar.value = new_hunger_ticks

func on_player_stamina_wait_timer_state_change(wait_time: float, started: bool) -> void:
	if started:
