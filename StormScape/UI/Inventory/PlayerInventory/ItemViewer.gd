...
@onready var elements_margin: MarginContainer = %ElementsMargin ## The margin container for all the info elements.
...
var item_hover_delay_timer: Timer = TimerHelpers.create_one_shot_timer(self, 0.45, _on_hover_delay_ended)
...
func _on_slot_hovered(slot: Slot) -> void:
	if pinned:
		...
		_update_item_title("Drop Here to Pin")
		return
	else:
...
func _on_mod_slot_changed(slot: ModSlot, old_item: InvItemResource, new_item: InvItemResource) -> void:
	...
	if item_viewer_slot.item.stats is WeaponResource:
		if old_item != null:
			WeaponModManager.remove_weapon_mod(item_viewer_slot.item.stats, old_item.stats, slot.mod_slot_index, GlobalData.player_node)
		...
		if new_item != null:
			...
			WeaponModManager.handle_weapon_mod(item_viewer_slot.item.stats, new_item.stats, slot.mod_slot_index, GlobalData.player_node)
			_assign_details(item_viewer_slot.item.stats)
...
func _on_item_viewer_slot_changed(_slot: Slot, _old_item: InvItemResource, new_item: InvItemResource) -> void:
	...
	if new_item == null:
		...
		_update_item_title(new_item.stats.name)
		item_rarity_label.text = GlobalData.invis_char + new_item.stats.get_item_type_string().capitalize() + " (" + new_item.stats.get_rarity_string().capitalize() + ") " + "[color=#93665800][char=02D9][/color]"
	...
	if (new_item.stats is WeaponResource) and (new_item.stats.max_mods > 0):
		_change_mod_slot_visibilities(true, new_item.stats.max_mods)
		var i: int = 0
...
func _change_mod_slot_visibilities(shown: bool, max_shown: int = -1) -> void:
	for slot: ModSlot in mod_slots:
		slot.item = null
		slot.is_hidden = not shown

		if slot.mod_slot_index + 1 > max_shown:
			slot.is_hidden = true

		mod_slots_margin.visible = shown

func _update_item_title(title: String) -> void:
	if title == "":
		elements_margin.visible = false
	else:
		elements_margin.visible = true
		item_name_label.text = GlobalData.invis_char + title + GlobalData.invis_char
