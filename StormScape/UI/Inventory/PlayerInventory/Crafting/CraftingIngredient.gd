...
@export var tags: Array[String] = [] ## The set of string tags that are allowed to match in order to be considered valid for this ingredient.
@export_enum("No", "Direct", "GEQ") var rarity_match: String = "No" ## What relative rarities are allowed to be used for this recipe. "No" means no rarity match required. "Direct" means exact match required. "GEQ" means greater or equal rarity match required.


func _to_string() -> String:
	return type.to_upper() + ": (" + str(quantity) + ") " + (item.get_recipe_id() if type == "Item" else str(tags)) + "[" + rarity_match + "]"
