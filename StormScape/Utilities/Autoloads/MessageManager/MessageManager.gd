...
@export var max_displayed: int = 10 ## How many messages can be displayed at once.
...
@onready var default_icon: Texture2D = load("uid://cjfmlylpqc505") ## The default icon used for messages.
...
func add_msg(text: String, text_color: Color = Globals.ui_colors.ui_light_tan,
				icon: Texture2D = default_icon, icon_color: Color = Color.WHITE,
				display_time: float = default_display_time, repeatable: bool = false) -> void:
	if message_stack.get_child_count() >= max_displayed:
		if msg_queue.size() >= max_queue_size:
			return
		elif active_msgs.has(StringName(text)) and not repeatable:
			return
		msg_queue.append({
			"text" : text, "text_color" : text_color,
			"icon" : icon, "icon_color" : icon_color,
			"display_time" : display_time, "repeatable" : repeatable
			})
		active_msgs[StringName(text)] = false
	else:
		if not active_msgs.get(StringName(text), false) or repeatable:
...
func add_msg_preset(text: String, preset: Presets, display_time: float = default_display_time,
					repeatable: bool = false) -> void:
	add_msg(text, presets[preset]["text_color"], presets[preset]["icon"], presets[preset]["icon_color"], display_time, repeatable)

## When a message exits the tree, clean it up and remove it from the active messages dictionary.
func _on_message_stack_child_exiting_tree(msg: HUDMessage) -> void:
	...
	if msg_queue.size() > 0:
		...
		add_msg.call_deferred(msg_details.text, msg_details.text_color, msg_details.icon, msg_details.icon_color, msg_details.get("display_time", default_display_time), msg_details.repeatable)
