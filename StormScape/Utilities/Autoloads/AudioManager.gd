		...
		if audio_stream and audio_stream is AudioStream:
			if audio_resource.has_available_stream():
				...
				if audio_resource.should_loop:
					...
					var callable = Callable(self, "_return_audio_player").bind(audio_player, audio_resource.name)
					audio_player.finished.connect(callable)
					audio_player.set_meta("callable", callable)
		...
		for i in range(validated_number_to_stop):
			if playing_sounds[i]: # to eliminate data race conditions
				if fade_out_time != 0.0:
					...
					_return_audio_player(playing_sounds[i], sound_name)
	else:
		for sound in playing_sounds:
			if sound: # to eliminate data race conditions
				if fade_out_time != 0.0:
					...
					_return_audio_player(sound, sound_name)
...
## Returns the audio stream player to its pool after removing it from its group, decrementing the audio 
## resource's active count, and removing the audio player's signals.
