...
## The default is to get the user key input, but this should be overridden by subclass states to determine how to move.
func _get_input_vector() -> Vector2:
	return Input.get_vector("move_left", "move_right", "move_up", "move_down").normalized()

## The default is to get the user key input, but this should be overridden by subclass states to determine when to dash.
