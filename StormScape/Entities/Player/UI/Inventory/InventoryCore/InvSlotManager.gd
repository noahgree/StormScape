...
@export var main_slot_grid: GridContainer ## The main container for all normal inventory slots. Can be null. Used when linking general purpose inventories (like from world chests) to this UI.
...
func _ready() -> void:
	...
	if synced_inv_src_node:
		fill_slots_from_synced_inv(synced_inv_src_node.inv.starting_inv.size())
...
func fill_slots_from_synced_inv(main_slot_grid_size: int) -> void:
	_setup_main_slot_grid(main_slot_grid_size)
...
## Fills the inventory slots with whatever the inventory has in its data.
...
func _on_inv_data_updated(index: int, item: InvItemResource) -> void:
	...
	if item and item.stats is ProjAmmoResource:
		if synced_inv_src_node is Player:
