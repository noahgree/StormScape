...
func _do_character_movement(delta: float) -> void:
	...
	if movement_vector == Vector2.ZERO:
		...
		if dynamic_entity.velocity.length() > max_speed * 0.10:
			fsm.anim_vector = dynamic_entity.velocity.normalized()
		else:
			fsm.anim_vector = movement_vector
		
		if request_sprint and stamina_component.use_stamina(fsm.sprint_stamina_usage * delta): # sprint
			fsm.anim_tree.set("parameters/run/TimeScale/scale", 1.5 * sprint_multiplier)
			dynamic_entity.velocity += (movement_vector * acceleration * sprint_multiplier * delta)
			dynamic_entity.velocity = dynamic_entity.velocity.limit_length(max_speed * sprint_multiplier)
		else: # move without sprint
			fsm.anim_tree.set("parameters/run/TimeScale/scale", 1.5)
			dynamic_entity.velocity += (movement_vector * acceleration * delta)
			dynamic_entity.velocity = dynamic_entity.velocity.limit_length(max_speed)
