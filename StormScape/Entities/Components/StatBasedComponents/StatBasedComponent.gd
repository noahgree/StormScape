...
func update_mod(stat_id: String, mod_id: String, new_value: float) -> void:
...
## Adds a mod to a stat. Handles logic for stacking if the mod can stack.
func add_mod(stat_id: String, mod: EntityStatMod) -> void:
	if stat_id in stat_mods:
		var existing_mod: EntityStatMod = stat_mods[stat_id].get(mod.mod_id, null)
		if existing_mod and existing_mod.can_stack:
			...
			stat_mods[stat_id][mod.mod_id] = mod
		
		_recalculate_stat(stat_id, base_values[stat_id])
	else:
		_push_mod_not_found_error(stat_id, mod.mod_id)
