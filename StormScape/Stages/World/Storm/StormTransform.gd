...
@export_custom(PROPERTY_HINT_NONE, "suffix:pixels") var new_radius: float = 100 ## The new radius to resize to.
...
static func create(loc: Vector2, offset_loc: bool, rad: float, offset_rad: bool, del: float, move_time: float,
	...
	st.new_location = loc
	st.offset_location = offset_loc
	st.new_radius = rad
	st.offset_radius = offset_rad
	st.delay = del
	st.time_to_move = move_time
